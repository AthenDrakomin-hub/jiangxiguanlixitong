...
export const menuAPI = {
  ...
  async getCategories(): Promise<DishCategory[]> {
    ...
    return await db.fetchCollection<DishCategory>('categories');
  },

  // 获取单个菜品分类
  async getCategory(id: string): Promise<DishCategory | null> {
    const db = await Database.getInstance();
    return await db.findById<DishCategory>('categories', id);
  },
  ...
  async updateCategory(id: string, updates: Partial<DishCategory>): Promise<DishCategory> {
    ...
    const existing = await db.findById<DishCategory>('categories', id);
    if (!existing) {
  ...
  async getDishes(query?: MenuQuery): Promise<Dish[]> {
    ...
    let dishes = await db.fetchCollection<Dish>('dishes');
  ...
  async getDish(id: string): Promise<Dish | null> {
    const db = await Database.getInstance();
    return await db.findById<Dish>('dishes', id);
  },
  ...
  async updateDish(id: string, updates: Partial<Dish>): Promise<Dish> {
    ...
    const existing = await db.findById<Dish>('dishes', id);
    if (!existing) {
  ...
  async deleteDish(id: string): Promise<boolean> {
    const db = await Database.getInstance();
    return await db.delete('dishes', id);
  },
  ...
  async initializeDefaultMenu(): Promise<void> {
    ...
    // 检查是否已有数据
    const existingCategories = await db.fetchCollection<DishCategory>('categories');
