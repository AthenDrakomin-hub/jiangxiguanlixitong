...
describe('QR Code Integration Tests', () => {
  ...
  it('should generate QR code URLs correctly', () => {
    const qrUrl = generateQRCodeUrl('http://localhost:5173', 'customer', '8201');
    
    expect(qrUrl).toContain('api.qrserver.com');
    expect(qrUrl).toContain('size=200x200');
    expect(qrUrl).toContain('data=http%3A%2F%2Flocalhost%3A5173%3Fpage%3Dcustomer%26id%3D8201');
  });
