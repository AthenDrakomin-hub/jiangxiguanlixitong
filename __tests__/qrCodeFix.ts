 ...
 */
function generateEnhancedQRUrl(
  basePath: string,
  page: string,
  id: string
): string {
  ...
  testCases.forEach((testCase, index) => {
    ...
    try {
      ...
      Object.defineProperty(window, 'location', {
        value: {
          search: testCase.url.includes('?')
            ? testCase.url.substring(testCase.url.indexOf('?'))
            : '',
          href: testCase.url,
        },
        writable: true,
      ...
      console.log(
        `${index + 1}. ${testCase.description}: ${pageMatch && idMatch ? 'PASS' : 'FAIL'}`
      );
      if (!pageMatch || !idMatch) {
        console.log(
          `   Expected: page=${testCase.expectedPage}, id=${testCase.expectedId}`
        );
        console.log(`   Got: page=${parsed.page}, id=${parsed.id}`);
        console.log(
          `   Routing result: currentPage=${routing.currentPage}, tableId=${routing.tableId}`
        );
      }
    } catch (error) {
