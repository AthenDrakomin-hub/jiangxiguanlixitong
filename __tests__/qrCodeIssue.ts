...
testCases.forEach((testCase, index) => {
  ...
  console.log(
    `${index + 1}. ${testCase.description}: ${result ? 'PASS' : 'FAIL'}`
  );
  if (!result) {
    console.log(
      `   Expected page: ${testCase.expectedPage}, got: ${parseUrlParams(testCase.url)['page']}`
    );
    console.log(
      `   Expected id: ${testCase.expectedTableId}, got: ${parseUrlParams(testCase.url)['id']}`
    );
  }
});
...
function generateQRCodeUrl(basePath: string, page: string, id: string): string {
  try {
    ...
    console.error('Error generating QR code URL:', _error);
    return ''; // Return empty string on error
  }
}

// Solution: Enhanced URL parameter parsing for CustomerOrder component
function initializeCustomerOrderParams(): {
  tableId: string;
  language: string;
} {
  try {
    ...
    console.error('Error initializing CustomerOrder params:', _error);
