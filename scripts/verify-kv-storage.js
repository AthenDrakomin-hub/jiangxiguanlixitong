...
async function verifyCollection(collectionName) {
  ...
  try {
    ...
    if (items.length !== index.length) {
      console.log(`⚠️  Inconsistency: ${items.length} items vs ${index.length} index entries`);
      return false;
    }
    ...
    console.error(`❌ Error verifying collection ${collectionName}:`, error.message);
    return false;
  }
}
...
async function runComprehensiveTests() {
  ...
  try {
    ...
    // Verify
    if (index.length !== testIds.length || !testIds.every(id => index.includes(id))) {
    ...
    if (!created.id || created.name !== testEntityData.name || created.value !== testEntityData.value) {
    ...
    // Update
    const updatedEntity = await kvClient.update(testEntity, created.id, { value: 84 });
...
async function main() {
  ...
  try {
    ...
    console.log('======================');
    console.log(`Collections verified: ${passedCollections}/${COLLECTIONS.length}`);
