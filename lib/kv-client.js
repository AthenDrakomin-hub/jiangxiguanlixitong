...
const redis = new Redis({
  url: process.env.HOTEL_KV_KV_REST_API_URL || process.env.HOTEL_KV_REST_API_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_URL,
  token: process.env.HOTEL_KV_KV_REST_API_TOKEN || process.env.HOTEL_KV_REST_API_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_TOKEN,
});

// Validate required environment variables
if (!(process.env.HOTEL_KV_KV_REST_API_URL || process.env.HOTEL_KV_REST_API_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_URL) || 
    !(process.env.HOTEL_KV_KV_REST_API_TOKEN || process.env.HOTEL_KV_REST_API_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_TOKEN)) {
  console.error('❌ Missing required Upstash Redis environment variables!');
  console.error('Please set: HOTEL_KV_KV_REST_API_URL and HOTEL_KV_KV_REST_API_TOKEN (or HOTEL_KV_REST_API_URL and HOTEL_KV_REST_API_TOKEN, or KV_REST_API_URL and KV_REST_API_TOKEN, or UPSTASH_REDIS_URL and UPSTASH_REDIS_TOKEN)');
...
export const kvClient = {
   ...
   */
  async addToIndex(entityType, id) {
    try {
      const indexKey = `${entityType}:index`;
   ...
   */
  async removeFromIndex(entityType, id) {
    try {
      const indexKey = `${entityType}:index`;
   ...
   */
  async create(entityType, itemData) {
    try {
   ...
   */
  async update(entityType, id, itemData) {
    try {
      const key = `${entityType}:${id}`;
