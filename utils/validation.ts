...
export const validateMinLength = (value: string, minLength: number, fieldName: string): ValidationError | null => {
  if (value && value.length < minLength) {
    return { field: fieldName, message: `${fieldName} 至少需要 ${minLength} 个字符` };
  }
  return null;
};
...
export const validatePhone = (phone: string, fieldName: string): ValidationError | null => {
...
export const validateNumber = (value: any, fieldName: string): ValidationError | null => {
...
export const validatePositiveNumber = (value: any, fieldName: string): ValidationError | null => {
