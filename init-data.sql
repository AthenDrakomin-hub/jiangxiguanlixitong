...
-- 清空现有数据（可选，取消注释以执行）
-- DELETE FROM kv_store WHERE key LIKE 'dishes:%' OR key LIKE 'hotel_rooms:%' OR key LIKE 'ktv_rooms:%' OR key LIKE 'payment_methods:%' OR key LIKE 'system_settings:%' OR key LIKE 'expenses:%' OR key LIKE 'sign_bill_accounts:%' OR key LIKE 'orders:%' OR key LIKE 'users:%' OR key LIKE 'inventory:%' OR key LIKE 'roles:%' OR key LIKE 'permissions:%' OR key LIKE 'hotel_reservations:%' OR key LIKE 'ktv_reservations:%' OR key LIKE 'daily_reports:%';
