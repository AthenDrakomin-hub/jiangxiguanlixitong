  ...
  const fetchAllData = useCallback(
    async (useCache: boolean = true) => {
      try {
        ...
        // Fixed: Properly cast unknown[] types from fetchAll to their respective types
        const appData: AppData = {
          dishes: (response.dishes as Dish[]) || [],
          orders: (response.orders as Order[]) || [],
          expenses: (response.expenses as Expense[]) || [],
          inventory: (response.inventory as Ingredient[]) || [],
          ktvRooms: (response.ktvRooms as KTVRoom[]) || [],
          signBillAccounts: (response.signBillAccounts as SignBillAccount[]) || [],
